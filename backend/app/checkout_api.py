...
from app.models import Address, Business, Delivery, Order, OrderItem, Payment, PaymentStatus, Product, ProductPresentation, Role, User
...
def quote_view(q):
    ...
    return {
        'business_id':b.id,'business_name':b.name,'currency':q['resolved'][0]['product'].currency if q['resolved'] else None,
        'coverage':q['coverage'],'delivery_method':q['delivery_method'],'distance_km':q['distance_km'],
        'delivery_radius_km':b.delivery_radius_km,'subtotal':q['subtotal'],'delivery_fee':q['delivery_fee'],'total':q['total'],
        'address':None if not a else {'id':a.id,'label':a.label,'line1':a.line1,'city':a.city,'province':a.province,'country':a.country,'latitude':a.latitude,'longitude':a.longitude},
        'items':[{'product_id':x['product'].id,'name':x['product'].name,'presentation_id':x['presentation'].id if x['presentation'] else None,'presentation_label':x['presentation'].label if x['presentation'] else None,'quantity':x['quantity'],'unit_price':x['unit_price'],'subtotal':x['subtotal']} for x in q['resolved']]
    }
...
def checkout_order(data:OrderIn,u:User=Depends(require(Role.CUSTOMER,Role.ADMIN,Role.SUPERADMIN)),db:Session=Depends(get_db)):
    q=quote_checkout(db,u,data.business_id,data.items,data.currency,data.delivery_method,data.address_id,lock=True)
    b=q['business']; a=q['address']
    delivery_address='' if not a else f'{a.line1}, {a.city}, {a.province}'
    o=Order(customer_id=u.id,business_id=b.id,address_id=a.id if a else None,currency=data.currency,subtotal=q['subtotal'],delivery_fee=q['delivery_fee'],total=q['total'],delivery_method=data.delivery_method,delivery_address=delivery_address,delivery_latitude=a.latitude if a else None,delivery_longitude=a.longitude if a else None,distance_km=q['distance_km'],customer_name=u.name,customer_phone=u.phone,notes=data.notes,payment_method=data.payment_method)
    db.add(o);db.flush()
    for x in q['resolved']:
    ...
    return {'id':o.id,'status':o.status.value,'payment_status':PaymentStatus.PENDING.value,'currency':o.currency,'subtotal':float(o.subtotal),'delivery_fee':float(o.delivery_fee),'total':float(o.total),'distance_km':o.distance_km,'delivery_method':o.delivery_method,'delivery_address':o.delivery_address}

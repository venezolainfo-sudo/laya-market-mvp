...
STATUS_LABELS={'NEW':'RECIBIDO','ACCEPTED':'ACEPTADO','PREPARING':'PREPARANDO','READY':'LISTO','COURIER_REQUESTED':'CADETE_SOLICITADO','ASSIGNED':'ASIGNADO','PICKED_UP':'RETIRADO','ON_THE_WAY':'EN_CAMINO','DELIVERED':'ENTREGADO','CANCELLED':'CANCELADO'}
ALLOWED={OrderStatus.NEW:{OrderStatus.ACCEPTED,OrderStatus.CANCELLED},OrderStatus.ACCEPTED:{OrderStatus.PREPARING,OrderStatus.CANCELLED},OrderStatus.PREPARING:{OrderStatus.READY,OrderStatus.CANCELLED},OrderStatus.READY:{OrderStatus.COURIER_REQUESTED,OrderStatus.ASSIGNED,OrderStatus.PICKED_UP,OrderStatus.DELIVERED,OrderStatus.CANCELLED},OrderStatus.COURIER_REQUESTED:{OrderStatus.ASSIGNED,OrderStatus.CANCELLED},OrderStatus.ASSIGNED:{OrderStatus.PICKED_UP,OrderStatus.CANCELLED},OrderStatus.PICKED_UP:{OrderStatus.ON_THE_WAY,OrderStatus.CANCELLED},OrderStatus.ON_THE_WAY:{OrderStatus.DELIVERED},OrderStatus.DELIVERED:set(),OrderStatus.CANCELLED:set()}
class StatusIn(BaseModel):status:OrderStatus
class CancelIn(BaseModel):reason:str=Field(min_length=3,max_length=500)
def get_business_for_user(db,u):
...
def order_detail(db,o):
    ...
    return {'id':o.id,'business_id':o.business_id,'business_name':b.name if b else '','customer_id':o.customer_id,'customer_name':o.customer_name,'customer_phone':o.customer_phone,'status':o.status.value,'status_label':STATUS_LABELS[o.status.value],'currency':o.currency,'subtotal':float(o.subtotal),'delivery_fee':float(o.delivery_fee),'total':float(o.total),'delivery_method':o.delivery_method,'delivery_address':o.delivery_address,'delivery_latitude':o.delivery_latitude,'delivery_longitude':o.delivery_longitude,'distance_km':o.distance_km,'payment_method':o.payment_method,'payment_status':pay.status.value if pay else 'PENDING','notes':o.notes,'cancellation_reason':o.cancellation_reason,'cancelled_by':o.cancelled_by,'created_at':o.created_at.isoformat(),'updated_at':(o.updated_at or o.created_at).isoformat(),'delivery':None if not delivery else {'id':delivery.id,'courier_id':delivery.courier_id,'status':delivery.status,'fee':float(delivery.fee)},'items':[{'id':i.id,'product_id':i.product_id,'presentation_id':i.presentation_id,'name':i.name,'presentation_label':i.presentation_label,'quantity':i.quantity,'unit_price':float(i.unit_price),'subtotal':float(i.subtotal)} for i in items]}
def apply_status(db,o,new):
    ...
    o.status=new;o.updated_at=datetime.utcnow();d=db.scalar(select(Delivery).where(Delivery.order_id==o.id));mapping={OrderStatus.COURIER_REQUESTED:'REQUESTED',OrderStatus.ASSIGNED:'ASSIGNED',OrderStatus.PICKED_UP:'PICKED_UP',OrderStatus.ON_THE_WAY:'ON_THE_WAY',OrderStatus.DELIVERED:'DELIVERED',OrderStatus.CANCELLED:'CANCELLED'}
    if d and new in mapping:d.status=mapping[new]
    add_notification(db,o,new.value)
def cancel_order(db,o,reason,actor):
    if o.status in {OrderStatus.DELIVERED,OrderStatus.CANCELLED}:raise HTTPException(409,'Este pedido ya no puede cancelarse')
    for i in db.scalars(select(OrderItem).where(OrderItem.order_id==o.id)).all():
    ...
    o.status=OrderStatus.CANCELLED;o.cancellation_reason=reason;o.cancelled_by=actor;o.updated_at=datetime.utcnow();p=db.scalar(select(Payment).where(Payment.order_id==o.id));d=db.scalar(select(Delivery).where(Delivery.order_id==o.id))
    if p and p.status==PaymentStatus.APPROVED:p.status=PaymentStatus.REFUNDED
    if d:d.status='CANCELLED'
    add_notification(db,o,'CANCELLED')
...
def merchant_orders(u:User=Depends(require(Role.MERCHANT,Role.MERCHANT_STAFF,Role.ADMIN,Role.SUPERADMIN)),db:Session=Depends(get_db)):
    b=get_business_for_user(db,u);return [order_detail(db,o) for o in db.scalars(select(Order).where(Order.business_id==b.id).order_by(Order.created_at.desc())).all()]
...
def merchant_notifications(u:User=Depends(require(Role.MERCHANT,Role.MERCHANT_STAFF,Role.ADMIN,Role.SUPERADMIN)),db:Session=Depends(get_db)):
    b=get_business_for_user(db,u);rows=db.scalars(select(Order).where(Order.business_id==b.id,Order.status==OrderStatus.NEW).order_by(Order.created_at.desc()).limit(10)).all();return {'new_order_count':len(rows),'orders':[{'id':o.id,'customer_name':o.customer_name,'total':float(o.total),'currency':o.currency,'created_at':o.created_at.isoformat()} for o in rows]}

...
STATUS_TEXT={
'NEW':('Pedido recibido','Tu pedido fue recibido por el comercio.'),
'ACCEPTED':('Pedido aceptado','El comercio aceptó tu pedido.'),
'PREPARING':('Preparando pedido','Tu pedido está siendo preparado.'),
'READY':('Pedido listo','Tu pedido ya está listo para salir.'),
'COURIER_REQUESTED':('Buscando cadete','Estamos asignando un cadete a tu pedido.'),
'ASSIGNED':('Cadete asignado','Ya hay un cadete asignado a tu pedido.'),
'PICKED_UP':('Pedido retirado','El cadete retiró tu pedido del comercio.'),
'ON_THE_WAY':('Pedido en camino','Tu pedido va en camino.'),
'DELIVERED':('Pedido entregado','Tu pedido fue marcado como entregado.'),
'CANCELLED':('Pedido cancelado','El pedido fue cancelado.')}

def add_notification(db:Session,o:Order,status:str|None=None):
    st=status or o.status.value;title,body=STATUS_TEXT.get(st,('Actualización de pedido',f'El pedido cambió a {st}.'))
    db.add(Notification(user_id=o.customer_id,order_id=o.id,kind='ORDER_STATUS',title=title,body=body))
...
def whatsapp_contact(order_id:str|None=None,u:User=Depends(current_user),db:Session=Depends(get_db)):
    ...
    phone=settings.whatsapp_business_number.replace('+','').replace(' ','')
    return {'phone':settings.whatsapp_business_number,'url':f'https://wa.me/{phone}?text={quote(text)}','automatic_cloud_api_ready':settings.whatsapp_cloud_ready}

@router.get('/notifications/my')
def my_notifications(u:User=Depends(current_user),db:Session=Depends(get_db)):
    rows=db.scalars(select(Notification).where(Notification.user_id==u.id).order_by(Notification.created_at.desc()).limit(100)).all()
    return [{'id':n.id,'order_id':n.order_id,'kind':n.kind,'title':n.title,'body':n.body,'read':n.read,'created_at':n.created_at.isoformat()} for n in rows]
...
def admin_assign(did:str,cid:str,u:User=Depends(require(Role.ADMIN,Role.SUPERADMIN)),db:Session=Depends(get_db)):
...
def tracking(oid:str,u:User=Depends(current_user),db:Session=Depends(get_db)):
    ...
    c=db.get(Courier,d.courier_id);loc=db.scalar(select(CourierLocation).where(CourierLocation.courier_id==c.id).order_by(CourierLocation.created_at.desc()))
    return {'order_id':oid,'status':o.status.value,'courier':{'id':c.id,'name':c.name,'phone':c.phone},'location':None if not loc else {'latitude':loc.latitude,'longitude':loc.longitude,'accuracy':loc.accuracy,'updated_at':loc.created_at.isoformat()}}

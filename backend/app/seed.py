...
def ensure_courier_user(db):
    u=db.scalar(select(User).where(User.email=='cadete@layamarket.local'))
    if not u:
        u=User(email='cadete@layamarket.local',password_hash=hash_password('Cadete123!'),name='Cadete Demo',phone='+54 261 0000001',role=Role.COURIER,country='AR');db.add(u);db.flush()
    c=db.scalar(select(Courier).where(Courier.user_id==u.id))
    if not c:
        c=db.scalar(select(Courier).where(Courier.phone=='+54 261 0000001'))
        if c:c.user_id=u.id
        else:db.add(Courier(user_id=u.id,name='Cadete Demo 1',phone='+54 261 0000001',country='AR',city='Mendoza'))
